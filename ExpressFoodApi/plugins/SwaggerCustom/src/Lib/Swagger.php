...
use SwaggerCustom\Lib\Schema\SchemaFromYamlFactory;
use Symfony\Component\Yaml\Yaml;

/**
 * Class Swagger
 *
 * @package SwaggerCustom\Lib
...
class Swagger
{
    ...
    /**
     * @var \SwaggerCustom\Lib\Model\ModelScanner
     */
    ...
    /**
     * @param \SwaggerCustom\Lib\Model\ModelScanner $modelScanner ModelScanner instance
     */
    public function __construct(ModelScanner $modelScanner)
    {
        ...
        EventManager::instance()->dispatch(
            new Event('SwaggerCustom.initialize', $this)
        );

        $xSwaggerCustom = Yaml::parseFile(self::ASSETS . 'x-swagger-bake.yaml');

        $this->array['x-swagger-bake'] = array_merge_recursive(
            $xSwaggerCustom['x-swagger-bake'],
    ...
    {
        EventManager::instance()->dispatch(
            new Event('SwaggerCustom.beforeRender', $this)
        );
    ...
    public function writeFile(string $output): void
    {
        if (!is_writable($output)) {
            throw new SwaggerCustomRunTimeException("Output file is not writable, given $output");
        }

        file_put_contents($output, $this->toString());

        if (!file_exists($output)) {
            throw new SwaggerCustomRunTimeException("Error encountered while writing swagger file to $output");
        }
    }
     ...
     *
     * @param \SwaggerCustom\Lib\OpenApi\Schema $schema Schema
     * @return $this
     */
     ...
     * @return \SwaggerCustom\Lib\OpenApi\Schema|null
     */
     ...
     *
     * @param \SwaggerCustom\Lib\OpenApi\Path $path Path
     * @return $this
     */
