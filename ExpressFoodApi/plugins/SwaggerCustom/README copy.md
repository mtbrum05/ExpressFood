# SwaggerCustom plugin for CakePHP4
...
[Demo Site](http://cakephpSwaggerCustom.cnizz.com/) | 
...
- [Multiple Instances of SwaggerCustom](#multiple-instances-of-swagger-bake)
...
SwaggerCustom requires CakePHP4 and a few dependencies that will be automatically installed via composer.

```console
composer require cnizzardini/cakephp-swagger-bake
```

Run `bin/cake plugin load SwaggerCustom` or manually load the plugin: 
...
```php
$builder->connect('/my-swagger-ui', ['controller' => 'Swagger', 'action' => 'index', 'plugin' => 'SwaggerCustom']);
``` 
...
I built this library to reduce the need for annotations to build documentation. SwaggerCustom will automatically 
...
SwaggerCustom works with your existing YML definitions and will not overwrite anything. 

## Doc Blocks

SwaggerCustom will parse your [DocBlocks](https://docs.phpdoc.org/latest/guides/docblocks.html) for information. The 
...
SwaggerCustom provides some optional Annotations for enhanced functionality. These can be imported individually from 
`SwaggerCustom\Lib\Annotation` or set to an alias such as `Swag`: `use SwaggerCustom\Lib\Annotation as Swag`.
[Read the Annotations docs](docs/annotations.md#SwaggerCustom-annotations) for detailed examples.
...
SwaggerCustom comes with an [event system](docs/events.md) to allow for further control over your 
OpenAPI schema.

| Event | Description | 
| ------------- | ------------- |
| [SwaggerCustom.Operation.created](docs/events.md) | Dispatched each time an OpenAPI Path > Operation is created |
| [SwaggerCustom.Schema.created](docs/events.md) | Dispatched each time an OpenAPI Schema is created |
| [SwaggerCustom.initialize](docs/events.md) | Dispatched during initialization phase on SwaggerCustom |
| [SwaggerCustom.beforeRender](docs/events.md) | Dispatched before SwaggerCustom outputs OpenAPI JSON |

## Customizing Exception Responses

By default, SwaggerCustom uses components > schemas > Exception as your Swagger documentations Exception schema. See the 
...
It's possible to write extensions for SwaggerCustom. Read the [extensions documentation](docs/extensions.md). There are 
several other options to extend functionality documented below:

#### Using Your Own SwaggerUI

You may use your own swagger install in lieu of the version that comes with SwaggerCustom. Simply don't add a custom 
...
3. Call SwaggerCustom programmatically: 

```php
$swagger = (new \SwaggerCustom\Lib\Factory\SwaggerFactory())->create();
...
SwaggerCustom comes with [Bake templates](templates/bake) for scaffolding RESTful controllers compatible with SwaggerCustom 
...
```
bin/cake bake controller {Name} --theme SwaggerCustom
```
...
- SwaggerCustom supports the following formats: application/json, application/x-www-form-urlencoded, application/xml, 
application/hal+json, and application/ld+json

SwaggerCustom does not document schema associations. If your application includes associations on things like 
GET requests, you can easily add them into your swagger documentation through the OpenAPI `allOf` property. Since 
SwaggerCustom works in conjunction with OpenAPI YAML you can easily add a new schema with this association. Below is an 
...
### SwaggerCustomRunTimeExceptions 
...
This is a new library so please take some steps before reporting issues. You can copy & paste the JSON SwaggerCustom 
...
Send pull requests to help improve this library. You can include SwaggerCustom in your primary Cake project as a 
